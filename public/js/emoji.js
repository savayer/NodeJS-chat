(function(){
    ...
    var k = function(){
        ...
        {
            return el.classList?el.classList.value.trim().split(' '):[];
        }
    ...
    function Emoji(EmojiElement, id, options)
    {
        this.id = id;
        var defaultOptions = {
            maxHeight: 200,
        ...
        k.addClass(EmojiElement, 'Emoji_Block');
        k.attr(EmojiElement, 'emoji-id', id);
/* 
        ...
        EmojiElement.appendChild(inputElementContainer); */
var inputElementContainer = document.querySelector('.controls');
/******************/
...
/******************/
        ...
        smilesContainerElement.classList.add('Emoji_Cont', 'Emoji_Cont_Hidden');
        ...
        for(var i in options.smiles)
            k.append(smilesContainerElement.firstElementChild, '<div class="Emoji_Smile" emoji="' + options.smiles[i] + '"><div><i class="ke ke-' + options.smiles[i] + '"></i></div></div>');

        k.append(smilesContainerElement.firstElementChild, '<div class="Emoji_Clear"></div>');

        EmojiElement.appendChild(smilesContainerElement);

        var bottomTollbarElement = document.createElement('div');
        k.addClass(bottomTollbarElement, 'smile_wrapper');

        var xmlns = "http://www.w3.org/2000/svg";
        var smileButton = document.createElementNS(xmlns, 'svg');
        smileButton.setAttributeNS(null, "viewBox", "0 0 32 32");
        smileButton.classList.add('smile');        
        
        var path = document.createElementNS(xmlns, "path");

        path.setAttributeNS(null, 'd', 'M15.988 4c-6.624 0-11.988 5.376-11.988 12s5.364 12 11.988 12c6.636 0 12.012-5.376 12.012-12s-5.376-12-12.012-12zM16 25.6c-5.304 0-9.6-4.296-9.6-9.6s4.296-9.6 9.6-9.6 9.6 4.296 9.6 9.6-4.296 9.6-9.6 9.6zM20.2 14.8c0.996 0 1.8-0.804 1.8-1.8s-0.804-1.8-1.8-1.8-1.8 0.804-1.8 1.8 0.804 1.8 1.8 1.8zM11.8 14.8c0.996 0 1.8-0.804 1.8-1.8s-0.804-1.8-1.8-1.8-1.8 0.804-1.8 1.8 0.804 1.8 1.8 1.8zM16 22.6c2.796 0 5.172-1.752 6.132-4.2h-12.264c0.96 2.448 3.336 4.2 6.132 4.2z');

        smileButton.appendChild(path)

        bottomTollbarElement.appendChild(smileButton);

        EmojiElement.appendChild(bottomTollbarElement);
        ...
        }([smilesContainerElement]));


        function insertSmileAtCursor(smile) {
            var img = document.createElement("IMG");
            k.attr(img, 'src', 'img/transparent.png');
            k.addClass(img, 'ke');
            ...
            if (window.getSelection) {
                ...
                if (sel.getRangeAt && sel.rangeCount) {
                    ...
                    if((currentInputElement == inputElement  | currentInputElement == inputElementContainer )
                        && currentBlockElement == EmojiElement){
        ...
        k.on(smileButton, 'click', function(){
            toggleSmiles();
        });
        ...
        function showSmiles()
        {
            ...
            smilesContainerElement.style.top = '65px';
            smilesContainerElement.style.left = 0;
            smilesContainerElement.style.display = "block";
        ...
        this.setWidth = function(width){
            EmojiElement.style.width = width + 'px';
        };

        this.setHeight = function(height){
            EmojiElement.style.height = height + 'px';
        };
        ...
        this.getValue = function(dataType){
            ...
            if(!dataType || dataType == Emoji.TEXT_VALUE)
                return val.replace(/<img.*?class="ke ke-(.*?)".*?>/g, " $#$1#$ ");
            else if(dataType == Emoji.HTML_VALUE)
                return val;
